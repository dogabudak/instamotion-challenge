import type { FormikFilters, HTTPFilterBody } from "../types/Filters";
...
export function buildFilters(filters: FormikFilters): {
  ...
  if (filters.color) {
    generatedFilters.color = filters.color;
  ...
  if (filters.mileage.from || filters.mileage.to) {
    generatedFilters.mileage = `["${filters.mileage.from || ""}", "${
      filters.mileage.to || ""
    }"]`;
    if (filters.mileage.from) {
      queryParams.mileFrom = filters.mileage.from;
    }
    if (filters.mileage.to) {
      queryParams.mileTo = filters.mileage.to;
    }
  }

  if (filters.power.from || filters.power.to) {
    generatedFilters.power = `["${filters.power.from || ""}", "${
      filters.power.to || ""
    }"]`;
  ...
  if (filters.price.from || filters.price.to) {
    generatedFilters.price = `["${filters.price.from || ""}", "${
      filters.price.to || ""
    }"]`;
